...
int bot_parse_action(struct IRC* bot, char* user, char* command, char* where,
    char* target, char* msg)
{
    // Private message example
    //	[from: Th3Zer0] [reply-with: PRIVMSG] [where: C-3PO_bot] [reply-to:
    // Th3Zer0] ciao
    // Channel message example
    //	[from: Th3Zer0] [reply-with: PRIVMSG] [where: ##freedomfighter]
    //[reply-to: ##freedomfighter] ciao
    if (DEBUG) {
        printf("[from: %s] [reply-with: %s] [where: %s] [reply-to: %s] \"%s\"",
            user, command, where, target, msg);
    }
    ...
    if (strcasecmp(argv[0], "ping") == 0) {
        ...
        if (argv[1] != NULL) {
            ...
            bot_raw(bot,
                "PRIVMSG %s :%s: you need to provide at least one arguments!\r\n",
                bot->chan, user);
        }
        ...
        if (argv[2] != NULL) {
            ...
            bot_raw(bot,
                "PRIVMSG %s :%s: you need to provide at least two arguments!\r\n",
                bot->chan, user);
        }
    } else if ((strcasecmp(argv[0], "archwiki") == 0) && argv[1] != NULL) {
        if (argv[2] != NULL) {
            bot_raw(bot,
                "PRIVMSG %s :%s: https://wiki.archlinux.org/index.php/%s\r\n",
        ...
        bot_raw(bot,
            "PRIVMSG %s :I'm a bot, developed using Coffee and Beer. You are "
            "%s, I think you know...\r\n",
            bot->chan, user);
        ...
        if (critical) {
            bot_raw(bot,
                "PRIVMSG %s :\001ACTION attacks %s! %d damage (it's super "
                "effective).\001\r\n",
        ...
        bot_raw(bot,
            "PRIVMSG %s :Information is power. But like all the power there "
            "are those who want to keep it for themselves -- ~Aaron Swartz\r\n",
            bot->chan);
        ...
        bot_raw(bot,
            "PRIVMSG %s :There is no future, go suck a bunch of dicks!\r\n",
            bot->chan);
    } else if (strcasecmp(argv[0], "privacy") == 0) {
        if (argv[1] != NULL) {
            bot_raw(bot,
                "PRIVMSG %s :%s: https://eff.org | https://prism-break.org | "
                "https://torproject.org | http://stallman.org\r\n",
                bot->chan, argv[1]);
        } else {
            bot_raw(bot,
                "PRIVMSG %s :%s: https://eff.org | https://prism-break.org | "
        ...
        if (argv[1] != NULL) {
            ...
            if (yt[0] == 'v' && yt[1] == '=') {
                yt = strtok(&yt[2], "&");
                bot_raw(bot,
                    "PRIVMSG %s :%s Click Here: "
                    "http://www.flvto.com/it/download/yt_%s/\r\n",
                    bot->chan, user, yt);
            }
        } else {
            bot_raw(bot,
                "PRIVMSG %s :%s Dowload and extract .mp3 from a video: "
                "`youtube-dl --extract-audio --audio-format mp3 link_here`\r\n",
                bot->chan, user);
        }
...
int bot_parse_service(struct IRC* bot, char* server, char* command, char* me,
    char* channel, char* msg)
{
    if (DEBUG) {
        printf("[server: %s] [command: %s] [me: %s] [channel: %s] %s\n", server,
            command, me, channel, msg);
    }
...
void bot_away(struct IRC* bot) { bot_raw(bot, "AWAY :C-3PO Bot\r\n"); }
// bot_quit: For quitting IRC
void bot_quit(struct IRC* bot) { bot_raw(bot, "QUIT :C-3PO Bot\r\n"); }
...
{
    if (cmd == NULL) {
        char h1[] = "!help !ping !quit !google !ddg !reddit !sqrt !archwiki "
                    "!whoami !attack !lookup !away !life !rms !random !privacy "
                    "!segfault !future <3";
        bot_raw(bot, "PRIVMSG %s :%s\r\n", bot->chan, h1);
